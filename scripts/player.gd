...
@export var top_speed: float = 125
@export var accel: float = 10
@export var friction: float = 15
@export var top_accel: float = 20
@export var top_friction: float = 25
...
var wing_scale: Vector2 = Vector2.ZERO
...
var sniper = Gun.new(3, 1, 0, 300, 0.3, 0, load("res://scenes/piercebullet.tscn"), 10, 25.0)
var clawgun = Gun.new(4, 1, 0, 150, 0.75, 0, load("res://scenes/bombbullet.tscn"), 2.5, 100.0)
var wand = Gun.new(5, 36, 360, 150, 2, 0, load("res://scenes/homebullet.tscn"), 1, 100.0)

var gun = pistol
var gunscale: Vector2 = Vector2.ZERO

func _ready() -> void:
	if Globals.playerded:
		Globals.powerups = Globals.saved_marker.duplicate()

	Globals.saved_marker = Globals.powerups.duplicate()
	Globals.playerded = false
	if Globals.level == 8:
		can_move = false
...
func _physics_process(delta: float) -> void:
	...
	if Globals.platformer:
		...
		$Wings.scale = lerp($Wings.scale, wing_scale, 1-(0.5**(10 * delta)))
		...
		if can_move:
			...
			if direction_vector != Vector2.ZERO:
				$anim.play("walk")
				velocity = lerp(velocity, (top_speed - (gun.weight*int(false))) * direction_vector, 1-(0.5**(top_accel * delta)))
			else:
				$anim.play("idle")
			...
			if Input.is_action_just_pressed("action") and $dash_timer.time_left == 0.0:
				$dash_timer.start()
				$dash.p()
				invincible = true
